...
; i j k func angle(deg) force\n""")
    ...
    def genbox_top(self, nchains, bpc, itpfilename, topfile):
        """Generate topology file for the box"""
        if isinstance(topfile, str):
            topfile = open(topfile, 'w')
        topfile.write("""; DO NOT EDIT - This file is automatically generated
; Generated by cgpoly on %s
...
[ system ]
%i BPAPC chains with %i beads each

[ molecules ]
BPAPC %i
""" % (time.asctime(), itpfilename, nchains, bpc, nchains))

    def gen_psf(self, nchains, bpc, psffile):
        """Generate PSF format topology (for VMD)

        From http://www.ks.uiuc.edu/Training/Tutorials/namd/namd-tutorial-unix-html/node21.html
       ...
       atom ID, segment name, residue ID, residue name, atom name, atom type, charge, mass, and an unused 0.
        ...
        f.write("""PSF
      2 !NTITLE
 REMARKS This file was automatically generated by cgpoly on
 REMARKS %s
%7i !NATOM
""" % (time.asctime(), nchains * bpc))
        for chain in range(nchains):
            for bead in range(bpc):
                ai = chain * bpc + bead + 1
                bn = bead % 4
                atype = ['P', 'C', 'P', 'I'][bn]
                aname = atype + str(bead + 1)
                resid = bead / 4 + 1
                resname = "M" + str(resid)
                f.write("%7i   PC %4i %4s %4s %4s  0.000000  73.0000      0\n"
                        % (ai, resid, resname, aname, atype))
        nbonds = nchains * (bpc - 1)
        f.write("%7i !NBOND\n" % nbonds)
        bonds_written = 0
        for chain in range(nchains):
            for bead in range(bpc - 1):
                ai = chain * bpc + bead + 1
                f.write(' %7i %7i' % (ai, ai + 1))
                bonds_written += 1
                if bonds_written % 4 == 0:
                    f.write('\n')
